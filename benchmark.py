...
matlab_command = "{} -log -nosplash -nodesktop -r \"try, options = odeset('RelTol'," + str(reltol) + ",'AbsTol'," + str(abstol) + "); fid=fopen('~benchmark-temp-{}.txt', 'w+'); for i = 1:{}, tic; {}(linspace({},{},{}), @ode15s, options); time = toc; fprintf(fid, '%.14f\\n', time); end, fclose(fid); catch me, disp(me.message); exit; end\""
...
for file_name in sbml_files:
				...
				for sim in time_map.keys():
								output += sim + " average:,,," + str(sum(time_map[sim][file_name])/trials) + "\n"
								output += sim + " minimum:,,," + str(min(time_map[sim][file_name])) + "\n"
								output += "\n"
